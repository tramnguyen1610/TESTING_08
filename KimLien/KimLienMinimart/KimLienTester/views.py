...
def api_get_nhaphang(request, ma_phieu):
    try:
        ...
        # Mapping dữ liệu Lô hàng để nạp vào giao diện Sửa
        sp_list = [{
        ...
        return JsonResponse({'status': 'success', 'data': {
            ...
            'SDT_NCC': pn.MaNCC.SDT_NCC if pn.MaNCC else '',
...
def api_search_sanpham(request):
    # Lấy các tham số từ giao diện gửi lên
    query = request.GET.get('q', '')
    status = request.GET.get('status', '')
    page_number = request.GET.get('page', 1)

    # Lấy toàn bộ sản phẩm, sắp xếp mới nhất lên đầu
    ...
    return JsonResponse({
        'results': results,
        'total_items': paginator.count,  # Tổng số lượng tìm được
...
def api_create_sanpham(request):
    if request.method == 'POST':
        ...
        if not ten_sp or not dvt:
            return JsonResponse({'status': 'error', 'message': 'Tên sản phẩm và Đơn vị tính không được để trống!'})
        if len(ten_sp) > 50:
            return JsonResponse({'status': 'error', 'message': 'Tên sản phẩm không được vượt quá 50 ký tự!'})

        try:
            # Tự động sinh mã sản phẩm tiếp theo (Vì Form Thêm đã ẩn ô Mã SP)
            last_sp = SanPham.objects.all().order_by('MaSP').last()
            if not last_sp:
                new_ma_sp = "SP001"
            else:
                match = re.search(r'(\d+)', last_sp.MaSP)
                new_ma_sp = f"SP{int(match.group(1)) + 1:03d}" if match else "SP001"
...
def api_get_sanpham(request, ma_sp):
    try:
        ...
        # 2. Tìm tất cả các lô hàng thuộc sản phẩm này
        lots = LoHang.objects.filter(MaSP=sp)

        # 3. Lấy tất cả các chi tiết nhập hàng liên quan đến các lô hàng đó
        # Chúng ta dùng select_related để lấy luôn thông tin Phiếu nhập và Nhà cung cấp trong 1 lần truy vấn
        import_details = ChiTietNhap.objects.filter(MaLH__in=lots).select_related('MaPhieu', 'MaPhieu__MaNCC')

        # TÍNH TOÁN TỒN KHO: Tổng số lượng nhập của tất cả các lô hàng sản phẩm này
        ton_kho = import_details.aggregate(Sum('SoLuongNhap'))['SoLuongNhap__sum'] or 0

        # LẤY GIÁ NHẬP & NHÀ CUNG CẤP GẦN NHẤT:
        # Sắp xếp theo ngày nhập của Phiếu nhập để lấy lần nhập mới nhất
        ...
        return JsonResponse({
            ...
            'data': {
                ...
                'GiaNhap': gia_nhap,  # Giá từ lần nhập mới nhất
                'TonKho': ton_kho,  # Tổng lượng nhập
                'TenNCC': ten_ncc  # Tên nhà cung cấp mới nhất
            }
        })
