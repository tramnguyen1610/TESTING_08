...
def dang_nhap_view(request):
    ...
    if request.method == 'POST':
        # Dùng .strip() của Python thay vì .trim() của JS
        ...
        if not u or not p:
            return render(request, 'DangNhap.html', {'error': 'Vui lòng nhập đầy đủ thông tin'})

        # Kiểm tra độ dài Tên đăng nhập
        if not (5 < len(u) < 20):
            return render(request, 'DangNhap.html',
                          {'error': 'Tên đăng nhập bắt buộc phải dưới 20 ký tự và hơn 5 ký tự.'})
...
def api_search_nhaphang(request):
    q = request.GET.get('q', ''); status = request.GET.get('status', ''); page = request.GET.get('page', 1)
    ...
    return JsonResponse({'results': results, 'total_items': paginator.count, 'current_page': page_obj.number, 'total_pages': paginator.num_pages})
