...
def api_search_ncc(request):
    ...
    nccs = NhaCungCap.objects.all().order_by('-pk')
    if q:
        # Bổ sung tìm kiếm theo SĐT
        nccs = nccs.filter(Q(TenNCC__icontains=q) | Q(MaNCC__icontains=q) | Q(SDT_NCC__icontains=q))
...
def api_search_nhaphang(request):
    ...
    month = request.GET.get('month', '')  # Lấy tham số tháng
    page = request.GET.get('page', 1)

    phieus = PhieuNhap.objects.all().order_by('-pk')

    if q:
        # Đã bổ sung tìm kiếm theo Mã NCC
        phieus = phieus.filter(Q(MaPhieu__icontains=q) | Q(MaNCC__TenNCC__icontains=q) | Q(MaNCC__MaNCC__icontains=q))

    if status and status != 'Tất cả trạng thái':
        phieus = phieus.filter(TrangThai=status)

    if month and month != 'Tất cả các tháng':
        # Lọc chính xác theo tháng của Ngày Nhập
        phieus = phieus.filter(NgayNhap__month=int(month))
    ...
    for pn in page_obj:
        results.append({
            ...
            'NgayNhap': pn.NgayNhap.strftime('%d/%m/%Y') if pn.NgayNhap else '',
            'TongTien': float(pn.TongTien or 0),
    ...
    if request.method == 'POST':
        try:
            ...
            if not ten_sp or not dvt:
                return JsonResponse({'status': 'error', 'message': 'Vui lòng điền tên và đơn vị tính!'})

            # Lấy sản phẩm cuối cùng
            last_sp = SanPham.objects.all().order_by('MaSP').last()

            if last_sp and last_sp.MaSP:
                # Dùng Regex chuẩn: r'(\d+)'
                match = re.search(r'(\d+)', last_sp.MaSP)
                if match:
                    next_number = int(match.group(1)) + 1
                    new_ma = f"SP{next_number:03d}"
                else:
                    new_ma = "SP001"
            else:
                new_ma = "SP001"
